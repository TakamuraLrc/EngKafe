...
function ReviewPayment() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-gray-900 mb-6">Review payment</h1>

        {/* Tabs - Below Title, Right Aligned */}
        <div className="flex justify-end gap-8 border-b border-gray-200 pb-4">
          <button
            onClick={() => setActiveTab('review')}
            className={`font-semibold text-lg transition-colors border-b-2 ${
              activeTab === 'review'
                ? 'text-pink-500 border-pink-500'
                : 'text-gray-600 border-transparent hover:text-gray-900'
            }`}
          >
            Review <span className="text-gray-400">({getTabCount('review')})</span>
          </button>
          <button
          ...
          >
            Approved <span className="text-gray-400">({getTabCount('approved')})</span>
          </button>
          <button
          ...
          >
            Denied <span className="text-gray-400">({getTabCount('denied')})</span>
          </button>
        </div>
      </div>
      ...
      {getFilteredPayments().length === 0 && (
        <div className="text-center py-12">
          <div className="text-gray-400 text-lg">No payments to display</div>
        </div>
      )}
