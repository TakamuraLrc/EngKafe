...
function Users() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-gray-900 mb-6">Manage user</h1>
        ...
        <div className="flex justify-between items-center">
          <div className="relative w-96">
            <input
              ...
              className="w-full px-4 py-2 pr-10 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-300"
            />
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-pink-200 border-b border-pink-300">
              ...
              <th className="px-6 py-4 text-left text-gray-900 font-semibold">Date created</th>
              <th className="px-6 py-4 text-left text-gray-900 font-semibold">Purchased Course</th>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <img
                        src={user.avatar}
                        alt={user.name}
                        className={`w-10 h-10 rounded-full object-cover ${!user.isActive ? 'opacity-50' : ''}`}
                      />
                      {!user.isActive && (
                    ...
                    <div>
                      <span className={`font-medium ${user.isActive ? 'text-gray-900' : 'text-gray-500'}`}>{user.name}</span>
                      {!user.isActive && <div className="text-xs text-red-500 font-semibold">Inactive</div>}
                    </div>
                  </div>
                </td>
                <td className={`px-6 py-4 ${user.isActive ? 'text-gray-700' : 'text-gray-400'}`}>{user.email}</td>
                <td className={`px-6 py-4 ${user.isActive ? 'text-gray-700' : 'text-gray-400'}`}>{user.dateCreated}</td>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-colors ${user.isActive ? 'bg-gray-800 text-white hover:bg-gray-900' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
                      title="Add Course"
                    >
                      <Plus size={18} />
                    </button>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={`p-2 rounded-lg transition-colors ${user.isActive ? 'text-green-600 hover:bg-green-100' : 'text-red-600 hover:bg-red-100'}`}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-900">Add {selectedUserForCourse?.name} to:</h2>
              <button
            ...
            <div className="p-6 max-h-96 overflow-y-auto">
              <div className="space-y-3">
                {defaultCourses.map((course) => (
                  <label key={course.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="text-gray-700 font-medium flex-1">{course.title}</span>
            ...
            <div className="flex gap-3 p-6 border-t border-gray-200 bg-gray-50 rounded-b-lg">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-pink-500 text-white font-medium rounded-lg hover:bg-pink-600 transition-colors"
              >
                Add
              </button>
            </div>
