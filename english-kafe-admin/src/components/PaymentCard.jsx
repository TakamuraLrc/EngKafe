  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-md border border-gray-200 hover:shadow-lg transition-shadow overflow-hidden">
        <div className="flex gap-4 p-6 pt-4 relative">
          {/* Left Side - Payment Slip */}
          <button
            onClick={() => setShowSlipModal(true)}
            className="w-32 shrink-0 relative group hover:opacity-80 transition-opacity"
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="flex items-start gap-4 mb-6 pb-4 border-b border-gray-200">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-500">{payment.userDate}</div>
              </div>
            </div>

            {/* Course Info */}
            <div className="mb-6 flex-1">
              <div className="text-gray-700 mb-1">{payment.courseName}</div>
              <div className="text-2xl font-bold text-gray-900">{payment.coursePrice}</div>
            </div>
            ...
            {status === 'review' && (
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2 rounded-lg font-semibold text-white bg-pink-500 hover:bg-pink-600 transition-colors"
                >
                  Deny
                </button>
                <button
                  onClick={() => onApprove(payment.id)}
                  className="flex-1 px-4 py-2 rounded-lg font-semibold text-white bg-green-600 hover:bg-green-700 transition-colors"
                >
                  Approve
                </button>
              </div>
            ...
            {status === 'approved' && (
              <div className="absolute bottom-4 right-6 bg-green-500 text-white px-4 py-1.5 rounded-full text-sm font-semibold">
                Approved
              </div>
            )}
            {status === 'denied' && (
              <div className="absolute bottom-4 right-6 bg-red-500 text-white px-4 py-1.5 rounded-full text-sm font-semibold">
                Denied
              </div>
            )}
          </div>
